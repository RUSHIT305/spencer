...
const { complete, ManagedGeminiError } = require('./gemini.js');
...
class Agent {
  constructor(settings, registry, options = {}) {
    ...
    this.provider = options.provider ?? { complete: (request) => complete(request, { timeoutMs: settings.apiTimeoutMs }) };
  ...
  async run(task) {
    ...
    for (let step = 1; step <= this.settings.maxSteps; step += 1) {
      ...
      if (!response?.choices?.length) throw new ManagedGeminiError('Managed Gemini returned no choices.');
      ...
      for (const call of toolCalls) {
        ...
        messages.push({ role: 'tool', name, tool_call_id: call.id ?? `tool-${step}`, content: String(result) });
      }
    }
